...
router.get("/pet/edit/:id", isShelter, async (req, res) => {
  try {
    ...
    pet.species === "dog"
      ? (options.speciesDog = true)
      : (options.speciesCat = true);
    ...
    const userMessages = pet.messages.reduce((newArray, message) => {
      if (
        !newArray.some(
          (messageUser) => messageUser.username === message.from.username
        ) &&
...
router.get("/view-messages/:idPet/:username", isShelter, async (req, res) => {
  try {
    ...
    const filteredMessages = pet.messages.filter((message) => {
      if (message.from.equals(user._id) || message.to.equals(user._id))
        return true;
      return false;
    });
...
router.post(
  "/message-send/:idPet/:idUserPerson",
  isShelter,
  async (req, res) => {
    try {
